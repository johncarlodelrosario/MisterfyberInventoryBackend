...
interface PopulatedInventoryRef {
  _id: string;
  name: string;
  price?: number;
  unit?: string;
...
const buildReportRows = async (
  ...
  for (const inst of installations) {
    const site = inst.siteId as unknown as PopulatedSiteRef | null;
    if (site?.name) {
      siteName = site.name;
    }
    ...
    for (const item of inst.items) {
      const inventory =
        item.inventoryId as unknown as PopulatedInventoryRef | null;
...
export const generatePDFReport = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (startDate || endDate) {
      const range = `${startDate ? new Date(startDate as string).toLocaleDateString() : "…"} - ${
        endDate ? new Date(endDate as string).toLocaleDateString() : "…"
      }`;
